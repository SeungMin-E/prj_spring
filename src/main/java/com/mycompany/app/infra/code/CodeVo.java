...
//	private int PageNoToShow = Constants.PAGE_NUM_TO_SHOW;
	...
	public int getThisIspage() {
		return thisIspage;
	}
	public void setThisIspage(int thisIspage) {
		this.thisIspage = thisIspage;
	}
...
//		setThisPage(3);
		...
		if(getTotalRows() == 0) {
			...
			setTotalPages(getTotalPages() / getRowNumToShow());
		}
		...
		if (getTotalPages() < getThisIspage() ) {
			setThisIspage(getTotalPages());
		}
		
		setStartPage(((getThisIspage() - 1) / getPageNumToShow()) * getPageNumToShow() + 1);
		...
		setEndRunmForOracle((getRowNumToShow() * getThisIspage()));
		...
		if(thisIspage == 1) {
			...
			setStartRunMForMysql((getRowNumToShow() * (getThisIspage() - 1)));
		}
		
		System.out.println("getThisisPage() : " + getThisIspage());
